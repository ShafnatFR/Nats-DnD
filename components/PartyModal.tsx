...
import { Character, Companion } from '../types';
...
const PartyModal: React.FC<PartyModalProps> = ({ party, onTalk, onDismiss }) => {
  if (party.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-[400px] text-slate-500">
        ...
        <p className="text-sm italic max-w-xs text-center font-crimson">
          "The path of the struggler is solitary... for now. Perhaps fate will bind others to your cause."
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-[500px]">
       <div className="mb-4 text-xs text-slate-500 uppercase tracking-widest font-cinzel text-center border-b border-slate-800 pb-2">
         Companions Bound by Fate ({party.length}/4)
       </div>
               ...
               <div className="flex-1">
                  <div className="flex justify-between items-start">
                     ...
                     <div className="flex gap-2">
                        <button 
                           onClick={() => onTalk(companion)}
                           className="p-2 bg-slate-800 rounded hover:bg-amber-900/40 text-stone-400 hover:text-amber-500 transition-colors"
                           title="Talk"
                        >
                           <MessageSquare size={16} />
                        </button>
                        <button 
                           onClick={() => onDismiss(companion)}
                           className="p-2 bg-slate-800 rounded hover:bg-red-900/40 text-stone-400 hover:text-red-500 transition-colors"
                           title="Dismiss"
                        >
                           <UserMinus size={16} />
                        </button>
                  ...
                  <div className="mt-3 space-y-2">
                     ...
                     <div>
                        <div className="flex justify-between text-[10px] text-slate-500 font-bold mb-0.5 uppercase">
                           <span className="flex items-center gap-1"><Heart size={10} /> Health</span>
                     ...
                     <div>
                        <div className="flex justify-between text-[10px] text-slate-500 font-bold mb-0.5 uppercase">
                           <span className="flex items-center gap-1"><Shield size={10} /> Loyalty</span>
