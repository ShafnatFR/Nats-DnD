...
import { Item, Character, Stats, Language } from '../types';
import { GET_UPGRADE_COST } from '../constants';
import { Hammer, Flame, ArrowRight, Anvil, CheckCircle, Shield, Sword } from 'lucide-react';
...
const SmithingModal: React.FC<SmithingModalProps> = ({ character, onUpgrade, language = 'EN' }) => {
            ...
            {upgradeableItems.map((item, idx) => (
               <button 
                  ...
                  className={`w-full text-left p-3 rounded border transition-all flex items-center gap-3 ${
                     selectedItem?.id === item.id ? 'bg-amber-900/20 border-amber-600' : 'bg-slate-900 border-slate-800 hover:border-slate-600'
               ...
               >
                  ...
                  <div className="flex-1">
                     <div className={`font-cinzel text-sm font-bold ${selectedItem?.id === item.id ? 'text-stone-200' : 'text-stone-400'}`}>
                        {getLocName(item, language as Language)}
                     </div>
         ...
         {selectedItem ? (
            <div className="flex flex-col h-full p-6 items-center z-10">
               <div className="text-center mb-8">
                  <h2 className="text-2xl font-cinzel font-bold text-stone-200 mb-1">{getLocName(selectedItem, language as Language)}</h2>
               ...
               {!isMaxLevel && cost ? (
                  <div className="w-full max-w-sm space-y-4">
                     <div className="grid grid-cols-2 gap-4">
                        <div className={`p-2 rounded border flex flex-col items-center ${hasEnoughMaterials ? 'bg-slate-950 border-slate-700' : 'bg-red-950/20 border-red-900'}`}>
                           ...
                           <div className="font-bold text-stone-300 text-sm mt-1">{getLocName(playerMaterial, language as Language) || "Unknown"}</div>
                        ...
                        <div className={`p-2 rounded border flex flex-col items-center ${hasEnoughGold ? 'bg-slate-950 border-slate-700' : 'bg-red-950/20 border-red-900'}`}>
                           ...
                           <div className="font-bold text-amber-500 text-sm mt-1">{cost.goldCost} G</div>
                           <div className={`text-xs font-mono mt-1 ${hasEnoughGold ? 'text-emerald-500' : 'text-red-500'}`}>
                              {character.gold >= cost.goldCost ? 'OK' : 'No Gold'}
                           </div>
                        </div>
                     </div>
                  ...
                  <div className="text-center p-8 border border-amber-500/30 bg-amber-900/10 rounded">
                     ...
                     <h3 className="text-amber-500 font-cinzel font-bold">Max Level</h3>
                  </div>
               )}
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-slate-600 opacity-50">
               ...
               <p className="font-cinzel">Select an item</p>
            </div>
         )}
      </div>
